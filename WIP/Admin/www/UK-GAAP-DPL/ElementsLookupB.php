...
'B' version of ElementsLookup.php to give presentation tree with ; separators and Tx fields for importing into Bros SS.
...
Remove concept id lookup?

History:
26.01.12 djh Started based on ElementsLookup120111.php
27.01.12 djh 'Ns' changed to 'Not set'
05.06.12 djh Revised for Bros change to TupId

From Charles:
Some is manual but quite quick e.g. working out the level

         [C] 1272 [H 29] [Money Dr Instant] Derivative financial instruments, asset
            [C] 1282 [H 29] [Money Dr Instant] Derivatives assets, held for trading
              [C] 1818 [H 29] [Money Dr Instant] Embedded derivatives, derivatives assets, held for trading
              [C] 2240 [H 29] [Money Dr Instant] Forwards & Futures, derivatives assets, held for trading
              [C] 3434 [H 29] [Money Dr Instant] Options, derivatives assets, held for trading
              [C] 1105 [H 29] [Money Dr Instant] Currency swaps, derivatives assets, held for trading
              [C] 4638 [H 29] [Money Dr Instant] Swaps, derivatives assets, held for trading

Tx;;;[C]1272 [H 29] [Money Dr Instant] Derivative financial instruments, asset
Tx;Tx;;1282 [H 29] [Money Dr Instant] Derivatives assets, held for trading
Tx;Tx;Tx;1818 [H 29] [Money Dr Instant] Embedded derivatives, derivatives assets, held for trading
Tx;Tx;Tx;2240 [H 29] [Money Dr Instant] Forwards & Futures, derivatives assets, held for trading
Tx;Tx;Tx;3434 [H 29] [Money Dr Instant] Options, derivatives assets, held for trading
Tx;Tx;Tx;1105 [H 29] [Money Dr Instant] Currency swaps, derivatives assets, held for trading
Tx;Tx;Tx;4638 [H 29] [Money Dr Instant] Swaps, derivatives assets, held for trading
*/
require 'BaseTx.inc';
...
Head("Bro Prep Elements Lookup: $TxName", true);

if (!isset($_POST['Input'])) {
  echo "<h2 class=c>Bro Data Prep Version of Lookup of $TxName Taxonomy Element(s)</h2>\n";
  $TypeB = $SpacesB = $NameB = true;
  $TreeChoice = 1;
  Form('');
}

$file = "/BrosAndTx/BroDataPrepElementsLookup-$TxName.txt";
echo "<h2 class=c>Bro Data Prep Version of Lookup of $TxName Taxonomy Element(s)</h2>
<p class=c>'Tx' ouput also to file <a href='Show.php?$file'>Admin$file</a> in tab delimited form.</p>
";
$Fh = fopen('../..'.$file, 'a');

$DataTypeFiltersA = array();
...
$NumAbstract  = 0;
$AbstractElsA = array();
$TupIdToTupTxIdA = array(); # TupId -> TupTxId
...
function Lookup($input) {
  global $DB, $TypeB, $SearchAB, $FilterByTypeB, $DataTypeFiltersA, $FromTreeB, $TreeChoice, $ShowBrosB, $ExclBrosB;
  $pHdg = '<p class=c>';
  if ($FilterByTypeB) {
    $list = '';
  ...
  $pHdg .= 'Tree element Types are indicated by: [A] = Abstract, [C] = Concrete, prefixed by S if in the Bro skip list'
    ...
    . ($TypeB ? ' [Data Type &lt;Not set|Debit|Credit> &amp; Period]' : '') . ' {[&lt;Start Label|End Label|StartEnd>]}.</p>';
  if (is_numeric($input)) {
    # assume E.id
    echo "\n<h2 class=c>TxId $input</h2>\n$pHdg\n";
    ...
    if ($res->num_rows) {
      ...
      $res = $DB->ResQuery($qry);
      if ($res->num_rows) {
        echo "\n<h2 class=c>" . ($SearchAB ? '' : 'Concrete ') . "Element(s) with Search String '$input'</h2>\n$pHdg\n<p class=c>" .
...
function ElementInfo($o) {
  global $DB, $NamespacesTxA, $TargetId, $FromTreeB, $TxElementsSkippedA, $StartEndTxIdsGA, $TargetIndent, $MaxIndent, $CRowsA;
  $elId = (int)$o->Id;
  $name = $o->name;
...
function ToTrees($toId) {
  ...
  $ToArcrolesA = array(); # to record arcs used by ToTrees() to exclude them from FromTrees() processing
  ...
  if ($res->num_rows) {
    ...
    while ($o = $res->fetch_object()) {
      ...
      $trees = array(array($id)); # trees[n][arrays of element Id]
      $lasti = array(0);     # last index (number of nodes -1) per tree
      $doneB = array(false); # set when tree is finished
      $anyB  = true;
      while($anyB) {
        $anyB = false;
        for ($ti=0; $ti<$nt; $ti++) {
      ...
      for ($ti=0; $ti<$nt; $ti++) {
        ...
        # echo "from 359 Select ToId,PrefLabelRoleId,TargetRoleId from Arcs Where ArcroleId=$arcroleId and PRoleId=$pRoleId and FromId=$id Order by TargetRoleId,ArcOrder<br>";
...
function TreeElement($id, $inset, $prefLabelRoleId=0, $targetRoleId = 0) {
  ...
  if ($o->abstract) {
    ...
    if ($TypeB) {
      if ($o->SubstGroupN == TSG_Tuple)
        ...
        if (($typeN = (int)$o->TypeN) == TET_Money)
          $typeS = ElementTypeToStr($typeN) . ($o->SignN ? ($o->SignN==1? ';Debit;' : ';Credit;') : ';Not set;') . PeriodTypeToStr($o->PeriodN);
        else
    ...
    $hysS = $o->Hypercubes ? (ChrListToCsList($o->Hypercubes)) : '';
    $txt = "{$b}[C] $id;$hysS;$tupIds;$typeS;$startEnd;$o->name";
    OutputTreeElement($id, $inset, $txt, $targetRoleId);
  }
}

# $MaxIndent used as the flag for current CRows. $TargetIndent serves to indicate we have had at atrget hit, could be multiple ones
function OutputTreeElement($id, $inset, $txt, $targetRoleId) {
  global $TargetId, $TargetIndent, $MaxIndent, $CRowsA;
  if ($targetRoleId)
    $txt .= ' (' . Role($targetRoleId) . ')';
  if ($id == $TargetId) {
    OutputCRows();
    $TargetIndent = $MaxIndent = $inset;
    $CRowsA[] = array($inset, $txt);
    return;
  }
  if ($MaxIndent && $inset > $TargetIndent) {
    if ($inset > $MaxIndent) $MaxIndent = $inset;
    $CRowsA[] = array($inset, $txt);
  }else{
    if ($MaxIndent) OutputCRows();
    #if (!$TargetIndent) { # output as usual until a target hit
      $pinset = 10;
      while ($pinset <= $inset) {
        echo '&nbsp;&nbsp;';
        $pinset += 12;
      }
      echo str_replace(';', '&nbsp;',$txt), "<br>\n";
    #}
  }
}

function OutputCRows() {
  global $TargetId, $TargetIndent, $MaxIndent, $CRowsA, $Fh;
  if ($MaxIndent) { # $MaxIndent used as the flag for current CRows. $TargetIndent serves to indicate we have had at atrget hit, could be multiple ones
    #echo "OutputCRows TargetIndent=$TargetIndent, MaxIndent=$MaxIndent, Num rows=", count($CRowsA), "<br>\n";
    fwrite($Fh, "TxId $TargetId ".gmstrftime('%Y-%m-%d_%H_%M').NL);
    foreach ($CRowsA as $i => $rowA){
      $inset = $rowA[0];
      $pinset = 10;
      $txt = '';
      while ($pinset <= $MaxIndent) {
        if ($pinset>= $TargetIndent)
          $txt .= ($pinset <= $inset ? 'Tx;' : ';');
        $pinset += 12;
      }
      $txt .= $rowA[1];
     #fwrite($Fh, str_replace(array(';','[C] '), array(TAB,''),$txt).NL);
      fwrite($Fh, str_replace(';', TAB,$txt).NL);
      if (!$i)
...
<tr><td class=r>Use Spaces rather than CSS for indenting Trees:</td><td><input class=radio type=checkbox name=Spaces disabled value=1$spacesChecked></td></tr>
