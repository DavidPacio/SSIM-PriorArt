...
Admin Utils/UK-GAAP-DPL/Arcs.php

Lists Arcs

History:
05.04.11 Started for UK-GAAP
08.10.12 Revised for UK-GAAP-DPL with TitleId removed
...
if ($res = $DB->ResQuery("Select A.*,E.name From Arcs A Join Elements E on E.Id=A.FromId $where")) {
  ...
  while ($o = $res->fetch_object()) {
    ...
    echo "<tr><td>$o->Id</td><td class=c>", Fold(LinkTypeToStr($o->TypeN), 5),
      "</td><td>E.{$o->FromId}<br>", Fold($o->name, 25), '</td><td>', To($o->TypeN, $o->ToId),
      '</td><td>', Role($o->PRoleId), '</td><td style="width:150px">', ArcroleIdToStr($o->ArcroleId),
      '</td><td>', Order($o->ArcOrder),
      '</td><td class=c>', Fold(UseTypeToStr($o->ArcUseN), 4), '</td><td>', $o->priority,
      '</td><td>', str_replace(' Label', '', Role($o->PrefLabelRoleId)),
      '</td><td>', BoolToStr($o->ClosedB), '</td><td>', Fold(ContextTypeToStr($o->ContextN), 4),
...
function To($typeN, $id) {
  ...
  switch ($typeN) {
    ...
    case TLT_Definition: return "E.$id<br>" . Fold(ElName($id), 25);
    case TLT_Label:
    case TLT_Reference:  return "D.$id<br>" . Fold($DB->StrOneQuery("Select T.Text from Doc D Join Text T on T.Id=D.TextId where D.Id=$id"), 25);
  }
  return $id;
}
