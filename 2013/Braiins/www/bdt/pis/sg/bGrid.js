 ...
 *     and do proper cleanup.
 ...
 djh changes
 - Shift Enter like Shift Tab
 - Tab and Enter movement wrap at top/bottom rows of grid
...
/* djh Removed for use with enableColumnReorder false
if (!jQuery.fn.drag) {
...
(function ($) {
   ...
   * @param {Object}            options     Grid options.
   djh: enableColumnReorder default changed from true to false
   **/
  function SlickGrid(container, data, columns, options) {
    ...
    //$headerScroller = $("<div class='slick-header ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container); djh style is in class
      $headerScroller = $("<div class='slick-header ui-state-default' />").appendTo($container);
      $headers = $("<div class='slick-header-columns' style='left:-1000px' />").appendTo($headerScroller);
      $headers.width(getHeadersWidth());

    //$headerRowScroller = $("<div class='slick-headerrow ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container); djh style is in class
    ...
    function handleKeyDown(e) {
      ...
      if (!handled) {
        ...
        } else if ((e.which == 9 || e.which == 13) && e.shiftKey && !e.ctrlKey && !e.altKey) { // 30.07.12 djh Shift Enter added
          navigatePrev();
        } else {
          return;
        }
    ...
    function gotoDown(row, cell, posX) {
      ...
      while (true) {
        ...
        //return null; 30.07.12 djh change to wrap
          row=0;
        }
        prevCell = cell = 0;
        while (cell <= posX) {
    ...
    function gotoUp(row, cell, posX) {
      ...
      while (true) {
        if (--row < 0) {
        //return null; 30.07.12 djh change to wrap
          row = getDataLength() + options.enableAddRow;
        }
    ...
    function gotoNext(row, cell, posX) {
      ...
      // 30.07.12 djh Replacing above to wrap
      var firstFocusableCell = null, gridLen=getDataLength() + options.enableAddRow;
      if (++row >= gridLen)
        row = 0;
      while (row < gridLen) {
        firstFocusableCell = findFirstFocusableCell(row);
