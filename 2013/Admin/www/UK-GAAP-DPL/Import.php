...
History:
08.05.11 Started
24.10.11 Set and member posting made mux
         With and without dim use made mux
30.10.11 Restated and other DiMes revised -> a lot DiMe entries created. See Data Dump.
01.11.11 Reworked ImportType and handling re DE checking on replacement of previous entries.
xx.11.11 Revised for Tuples, StartEnd, more BD Map properties, ....
xx.12.11 More for Tuples and StartEnd
08.12.11 Changed from comma to tab separated for simplicity re strings containing commas, to allow thou seps in numbers, and for consistency re Bros Export/Import
23.01.12 Revised for DB changes re Year
10.04.12 Check revised to avoid repeating the check fail for multiple dim refs
21.06.12 Revised for Bro changes re Tuples, Slaves and TypeN->Bit settings
24.07.12 Handling of ()s for -ve numbers added.
26.07.12 Added option of pasting the import data
02.08.12 Removed utf-8 BOM from file read if present
         Started on multiple pass processing and table layout reporting
06.08.12 Changed import type Additional to Update
         DE Sums and Processed BroRef if different added to Pass 2 table.
xx.08.12 Output -> tables, lots of incremental changes with a more complete sets of Bros and Import data from Charles' work, Master/Slave processing, StartEnd processing.
xx.09.12 Continuing...
08.09.12 CheckId -> CheckTest
15.09.12 Revised for DiMes (BroDiMesA) vs DiMeId
20.09.12 Added Prior Year Slave handling when copying masters to Slaves.
22.09.12 Line comment ability added; empty line output limited to one at a time
23.09.12 PostEnd calcs added; Year# option added to Check
24.09.12 Use of BroDI_BroId removed
26.10.12 Switched to use of Class Bro
14.11.12 Determination of $setMaxId moved to BuildStructs.php and BroInfoA[BroI_SetMaxId]
23.11.12 Class Bro Tuples Extension working
24.11.12 Started on getting PYA working again; made it illegal to have mixture of Std and PYA postings in one run.
14.12.12 Removed Update option. Will use a separate module for that when needed.
24.12.12 Changed to delete PYA Year data if the corresponding standard year is re-imported
         Removed all BrosAll updating
24.01.13 Removed Set Slave storage in DB with Set Slave sum with no filtering != Master => error
...
A Bro reference takes the form of a Bro Name optionally followed by any number of dimension member references and/or an end|start property, and/or a tuple reference in any order.
...
# $ETypeId, $ETypeChr, $GlobalDimGroupExclDimsChrList, and $ESizeId to have been set
# $BroInfoA, $BroNamesA, $BroNamesMapA, $DiMeMapA, $DiMesA, $DiMeNamesA, $DimGroupsA to have been loaded
...
require Com_Str_Tx.'DiMeMapA.inc';     # $DiMeMapA
...
Bro::SetNewPostingDiMeBroDatCallbackFn('SetDimGroups'); # To callback SetDimGroups when a new posting DiMe BroDat is added.
...
# Get Entity Info
$EA = $DB->AaQuery("Select ETypeId,ESizeId,CurrYear,DataState,DGsInUse,DGsAllowed From Entities Where Id=$EntityId");
foreach ($EA as $k => $v)
  $$k = (int)$v; # -> $ETypeId, $ESizeId, $CurrYear, $DataState, $DGsInUse, $DGsAllowed as ints
...
$Errors = $numStdYears = $startYear = $startYearB = $pyaImportB = $firstDatLen = $NoDEcheck = 0; # $startYearB so that a second StartYear: # command can be detected.
$Debug = $IDebug;
...
foreach ($LinesA as $LineNum => &$lineR) {
  ...
  # BroName{:DimRef...}{:<end|start>}{:T{.#}}	Value(s)
  # |-------------- BroRef -----------------|-- v --...
  if (($broRefA = BroRefPost($colsA[0])) !== false) {  # BroRefPost() returns [BroId, DiMeIdsA, RefBits, Inst] or false, with DiMeIdsA = 0 if no DiMes i.e. not []
    $BroRefsA[$LineNum] = $broRefA;
    $years = count($colsA)-1;
    $pya = $broRefA[2] & BRefB_DiMePya; # $broRefA[2]=RefBits
    if ($pyaImportB) {
      # Pya year import
      if ($years > 1)
...
if ($retrievePrevYearsiA) { # All years except for replace years. Unlikely to be empty but possible
  ...
  while ($o = $res->fetch_object()) {
    ...
    $brO   = NewBroFromString($broId, $o->BroStr); # SetDimGroups() is called if appropriate via the callback fn. Includes DiMe Summing which can be needed for 'd' PyaOA values.
    ...
    if (!$importYearsA[$year]) {
      ...
      $broYearsA[$year] = 1;
      if ($brO->IsTuple())
        # Record the tuple instance use
        # $tupInstsA   = # Tuple Instances in use [BroId => [year => [i => Inst]]]
        foreach ($brO->PostingBroDatOs() as $datO)
          $tupInstsA[$broId][$year][] = $datO->Inst;
    }
    if ($brO->IsMaster()) {
...
foreach ($BroRefsA as $lineNum => $broRefA) { # $BroRefsA = Processed BroRefs for data rows [LineNum => BroRefA]
  ...
  list($broId, $diMeIdsA, $refBits, $inst) = $broRefA;# [BroId, DiMeIdsA, RefBits, Inst] with DiMeIdsA = 0 if no DiMes i.e. not []
  ...
  $broRef = $diMeIdsA ? "$broId,".implode(',', $diMeIdsA) : $broId;
  ...
  if ($broA[BroI_TuMeId]) {
    # Tuple Bro
    # Inst will have been defined, no instance being an error in Pass 1.
    if (isset($tupInstsA[$broId][$startYear]) && in_array($inst, $tupInstsA[$broId][$startYear])) {
      StackMsg("Tuple instance $inst is already in use with Bro $broId $BroNamesA[$broId]");
      $inst = -1; # flag to avoid adding data to the Bro without a valid Inst which would give another error
    }
  }
    ...
    if (strlen($dat)) {
      ...
      if (($refBits & BRefB_Start) && $priorYearsA[$year]) {
       ...
       #$prevBrosRA = &$PrevBrosA[$year]; # previous Bros for the year  [BroId => Bro] with array empty if none
        switch ($dataTypeN) {
          ...
          case DT_Share:
            if (ctype_digit($dat))
              ...
              StackMsg(sprintf("Year %s <i>$dat</i> value not valid numeric data for Bro <i>$bName</i> which has a data type of %s", YearStr($year), DataTypeStr($dataTypeN)));
            $src=$dat;
            break;
          case DT_Money:
            if (($dat=FormattedNumberToInt($dat)) !== false) { # FormattedNumberToInt() changes enclosing ()s to leading - and strips thou seps
              $src=$dat;
             #if ($deBro && in_array(DiMeId_HighestPaidDirector, $diMeIdsA)) { 16.08.12 fudge for DiMeId_HighestPaidDirector postings not to be treated as DE but not kept following "Import Data Report Directors Report Remuneration Analysis Highest Paid Director" email exchange with Charles
              if ($deBro) {
              ...
              StackMsg(sprintf("Year %s <i>$src</i> value not valid integer numeric data for Bro <i>$bName</i> which has a data type of money", YearStr($year)));
              if ($deBro) {
                if ($skip) $deh .= "<td colspan=$skip></td>";
                $deh .= '<td></td>';
              }
          ...
          case DT_Date:
            if (!$dat = StrToDate($dat))
              StackMsg(sprintf("Year %s date not valid for Bro <i>$bName</i> which has a date data type", YearStr($year)));
            break;
          case DT_Boolean:
            switch (strtolower($dat)) {
              ...
              default: StackMsg(sprintf("Year %s requires one of 0, n, no, false, 1, y, yes, true (case insensitive) as Bro <i>$bName</i> has a data type of boolean", YearStr($year)));
            }
            break;
          default: StackMsg("Unknown DataTypeN $dataTypeN for Bro $bName");
        }
        if ($yrBrOsRA) {
          # Check for Mux unless first entry in year. All Bros (entries) are primary at this point except for BroDatSrce_b b Base = sum of DiMes ones.
          # #############
          # Bro Set Mux
          # -----------
          ...
          # DiMe Mux
          # --------
          if ($refBits & BRefB_DiMeMux)  # if this entry incudes a DiMe that is part of a Mux List
            foreach ($diMeIdsA as $diMeId) # for all the entry's DiMes
              if ($DiMesA[$diMeId][DiMeI_MuxListA]) # if this DiMe has a Mux List
                foreach ($DiMesA[$diMeId][DiMeI_MuxListA] as $muxDiMeId) # for each of the Mux list DiMes
                  foreach ($yrBrOsRA as $broId2 => $brO2)    # for each Bro in the year so far
                    foreach ($brO2->AllPrimaryDiMeBroDatAs() as $datA) # Through all the Primary DiMes with Tuples amalgamated as [0 => Bal, 1 => DiMeIdsA]
                      foreach ($datA[1] as $diMeId2)  # for each DiMe in the prior DiMe posting
                        if ($diMeId2 == $muxDiMeId)
                          StackMsg(sprintf("Year %s posting using dimension {$DiMeNamesA[$diMeId]} ($diMeId) is invalid as a posting to <i>%s</i> uses the mutually exclusive Dimension Member {$DiMeNamesA[$diMeId2]} ($diMeId2)", YearStr($year), $brO2->BroRefFull()));
          /* 24.11.12
          # NoDiMe vs DiMe Mux
          # ------------------
          if ($diMeIdsA) {
        ...
        # Add to $BrosA[] as one BroDat with all DiMes. SetDimGroups() is called if appropriate via the callback fn, unless Inst has not been determined
        if ($inst >= 0) {
          if ($inst)
            $tupInstsA[$broId][$year][] = $inst;
          if (!isset($yrBrOsRA[$broId]))
            $yrBrOsRA[$broId] = new Bro($broId);
          $broDatO = $yrBrOsRA[$broId]($broDatType, $dat, BroDatSrce_P, $inst, $diMeIdsA, $op); # add data ($broDatTypeOrBroDatKey, $dat, $srceN, $inst=0, $diMeIdOrDiMeIdsA=0, $op=BroAddDataOp_Unique) Any duplicate posting errors will be stacked via Bro ErrorCallbackFn = 'StackMsg';
        }else
          $broDatO = false;
  ...
  $rowh .= ($procBroRef == $srceBroRef ? '<td></td>' : "<td class=l>$procBroRef</td>");
...
foreach ([6,3,5,2,4,1,0] as $year) {
  ...
  foreach ($BroSumTypesGA as $dataTypeN) { # [DT_Integer, DT_Money, DT_Decimal, DT_Share] = [2,3,4,8];
    ...
    # Build $BrosThisDataTypeA as an array of Bro use for this data type: [BroId => 1]
    $BrosThisDataTypeA = null;
    foreach ($yrBrOsA as $broId => $brO)
      if ($brO->DataTypeN === $dataTypeN)
    ...
    if ($dataTypeN === DT_Money && $priorYear = $priorYearsA[$year]) {
      ...
      foreach ($BrosA[$priorYear] as $seBroId => $priorSeBrO) {
        if ($priorSeBrO->IsStartEnd() &&  # is StartEnd Bro
          ...
          foreach ($priorSeBrO->EndPrimaryBroDatOs() as $datO) { # [BroDatKey => BroDatO] of End Primary BroDats incl tuples
            $seBrO = isset($yrBrOsA[$seBroId]) ? $seBrO = $yrBrOsA[$seBroId] : $yrBrOsA[$seBroId] = new Bro($seBroId);
            $BrosThisDataTypeA[$seBroId] = 1;
            $seBrO($datO->BroDatType - BroDatT_Start, $datO->Dat, BroDatSrce_PE, $datO->Inst, $datO->DiMeIdsA); # add data to Bro as a Start BroDat ($broDatTypeOrBroDatKey, $dat, $srceN, $inst=0, $diMeIdOrDiMeIdsA=0, $op=BroAddDataOp_Unique)
    ...
    if (!$BrosThisDataTypeA)
      continue; # No data to sum for this data type
    ...
    if ($dataTypeN === DT_Money) {
      ...
      foreach ($SumEndBrosA as $tarBroId => $sumListA) { # [BroId => [i => BroId to sum]]
        # SumEnd Bro
        ...
        if (isset($yrBrOsA[$tarBroId]))
          foreach ($yrBrOsA[$tarBroId]->StartPrimaryBroDatAs() as $amalBroDatKey => $datA) {  # [AmalBroDatKey => [0 => Bal, 1 => SrceN]] for the Start Summing BroDats i.e. excluding the End, Base one(s) if there are DiMes or Tuple instances, non-Primary ones, and in a Pya year, all non-Pya BroDats.
            AdjustAmalBroDatKey($amalBroDatKey, BroDatT_Start); # Start -> normal i.e. End
            $sumA[$amalBroDatKey] = $datA; # [AmalBroDatKey => [0 => Bal, 1 => SrceN]]
            if ($Debug) DebugMsg("SumEnd Summing year $year $tarBroId,$amalBroDatKey Bal $datA[0] created");
          }
        # Now the SumList
      ...
      */
      ...
      foreach ($PostEndBrosA as $seBroId => $listA) {
      ...
      foreach ($StockBrosA as $seBroId => $mvBroId) # [BroId => Stock Movement BroId]. Only ever one of them.
        if (!($BroInfoA[$seBroId][BroI_Bits] & BroB_Set)) {
          ...
          if ($mvtA = Movement($seBroId)) { # false if nothing
            ...
            foreach ($mvtA as $broDatKey => $bal) { # End Base or End DiMe, no Start
              $bal = -$bal;
 ...
 /* Need to copy masters to slaves
    ...
    for ($sumPass=1; true; ++$sumPass) { # Infinite loop check based on $sumPass=== 9 is done at the end
     ...
     #foreach ($BrosThisDataTypeA as $broId => $t) No because could have a prior year master re Year Slaves that is not a master in this year.
      ...
      if ($treeA !== 0) # Check that there is a summing tree for this data type
    ...
    foreach ($PostEndBrosA as $seBroId => $listA) { # [BroId => [i => BroId]]
    ...
    foreach ($StockBrosA as $seBroId => $broId) { # [BroId => Stock Movement BroId]
      # Stock
      if (isset($yrBrOsA[$seBroId])) {
        $seBrO= $yrBrOsA[$seBroId];
        $startSum = $seBrO->StartBase();
    ...
    } # end of check equivalences loop

    ################
    # DiMe Summing # which includes Intermediate DiMe Summing and Pya DiMe Summing if applicable
    ################
    Debug("DiMe Summing year $year");
    $stdYear = $year - Pya_Year_Offset; # > 0 if applicable i.e. in a Pya year
    foreach ($BrosThisDataTypeA as $broId => $t)
      $yrBrOsA[$broId]->DiMeSumming($stdYear>0 ? (isset($BrosA[$stdYear][$broId]) ? $BrosA[$stdYear][$broId] : null) : null); # Passed the StdYear BrO for a Pya year, or null if not applicable or not defined
    Debug();
...
foreach ($sumYearsA as $year => $b) { # year is relYear
  ...
  # Loop to Insert the Bros. Also does C.1 and C.2 for ImportType:Complete and ImportType:Update
  # C.1 If an import Bro matches a previous Bros (all BroDats), the previous Bro is left untouched.
  # C.2 If an import Bro differs from a previous Bro, the previous Bro is updated (replaced)
  #     and a BroTrans table BDTT_ImportUpdate (Audit Trail) tran generated.
  foreach ($yrBrOsRA as $broId => $brO) {
    ...
    if ((!$importYearB || $importType === IT_Complete) && isset($prevBrosRA[$broId])) {
        ...
        if ($importYearB && ($brO->HasPosting() || $prevBrO->HasPosting())) {
          ...
          foreach ($prevDatOsA as $broDatKey => $datO) {
            if (isset($thisDatOsA[$broDatKey])) {
              ...
              #   DatType {DiMeRef} Dat
              #   DatType Inst{DiMeRef} Dat
              if ($brO->IsTuple())
                $broDatsA[$broDatKey] = (is_int($broDatKey) || strlen($broDatKey)===6) ? "$this->BroDatType$this->Inst" : "$this->BroDatType$this->Inst".implode(',', $this->DiMeIdsA).'';
              else
         ...
         #uksort($broDatsA, 'strnatcmp');
          ksort($broDatsA, SORT_NATURAL);
          $colsAA = [
            'BrosId' => (int)$pA['Id'],
            'MemId'  => $DB->MemId,
            'TypeN'  => BDTT_ImportUpdate,
            'TransSet' => implode('', $broDatsA)];
          $DB->InsertMaster(T_B_BroTrans, $colsAA);
        }
      ...
      $brosId = $DB->InsertMaster(T_B_Bros, $colsAA);
...
$colsAA = [];
# Bits for Entities.DataState
#onst EDSB_NotRun     0  Bit
#onst EB_DS_OK       = 1; # 0 OK
#onst EB_DS_Errors   = 2; # 1 Critical errors found. Not used here as data isn't imported in such cases.
#onst EB_DS_Warnings = 4; # 2 Warnings found
$actioN2 = $info2 = 0;
$dataState = $WarningsA['Num'] ? $DataState | EB_DS_Warnings : EB_DS_OK;
if ($dataState != $DataState) {
  $colsAA['DataState'] = $dataState;
...
# $sumA = [AmalBroDatKey => [0 => Bal, 1 => SrceN]]
...
function SumBros($tarBroId, $broIdsA, $sumType, $sumA=[]) { # $sumA=[] for a Set summing op with base initialised to false
  global $DiMesA, $yrBrOsA, $BrosThisDataTypeA, $year, $Debug;
  ...
  foreach ($broIdsA as $broId) {   # thru the Bros that sum to the target $tarBroId
    if (isset($yrBrOsA[$broId])) { # There is data for this $broId. Sum to the target $sumA[]
      foreach ($yrBrOsA[$broId]->SummingBroDatAs() as $amalBroDatKey => $datA) { # [AmalBroDatKey => [0 => Bal, 1 => SrceN]] for the Summing BroDats i.e. excluding the Base one(s) if there are DiMes or Tuple instances, non-Primary ones, and in a Pya year, all non-Pya BroDats. Should only be called for a Non-StartEnd Summing Bro. For Tuples, the Inst # is stripped from the key and same key results wo Inst are summed.
       #echo "amalBroDatKey |$amalBroDatKey| => [{$datA[0]},{$datA[1]}]<br>";
        $bal = $datA[0];
        if (isset($sumA[$amalBroDatKey])) {
          $sumA[$amalBroDatKey][0] += $bal;
          if ($Debug) {$sumTypeS = $sumType==BroDatSrce_S ? 'Set' : 'SumEnd'; DebugMsg("$sumTypeS Summing year $year $tarBroId,$amalBroDatKey Bal {$sumA[$amalBroDatKey][0]} after adding Bro $broId,$amalBroDatKey bal $bal");}
        }else{
          if (is_string($amalBroDatKey)) ++$numDiMesA[(int)$amalBroDatKey];
          $sumA[$amalBroDatKey] = [$bal, BroDatSrce_S]; # [BroDatKey => [0 => Bal, 1 => SrceN]]
          if ($Debug) {$sumTypeS = $sumType==BroDatSrce_S ? 'Set' : 'SumEnd'; DebugMsg("$sumTypeS Summing year $year $tarBroId,$amalBroDatKey Bal $bal created from Bro $broId,$amalBroDatKey bal $bal");}
        }
      }
    }
  }
  ...
  # Got summed data
  # If there a Base sum or sums and there are DiMes involved, then the Base or Bases need adjustment.
  # the target Bro's Mandatory/Default DiMe if it has one, o'wise to DiMeId_Unallocated
  $tarBrO = isset($yrBrOsA[$tarBroId]) ? $yrBrOsA[$tarBroId] : $yrBrOsA[$tarBroId] = new Bro($tarBroId);
  $tarUsableDims = $tarBrO->InfoA[BroI_SumUsableDims];
  if ($tarUsableDims && ($numDiMesA[BroDatT_End] || $numDiMesA[BroDatT_Start])) {
    # Target has Usable Dims and DiMes are involved, so check that that DiMes in the sum are OK for the target's usable dims.
    # have 1 or more DiMe sums
    # First check if target has Group (dimId == 1) but sum has a dim Consol (dimId == 2) balance, and vice versa.
    # which is OK if the DiMe is Consol or Consol.Consol (DiMeIds 3,4) which are the same as Group.Consol or Group (DiMeIds 2,1)
    # and if so change the Consol 3,4 sum to Group 2,1
    if ($tarUsableDims[0]==='1') { # will be first if present as usable dims are sorted
      foreach ($sumA as $amalBroDatKey => $sumDatA) # [0 => Bal, 1 => SrceN]
        if (is_string($amalBroDatKey)) { # only do DiMes i.e. skip StartBase and Base sums
          # '3' -> '2', or 'x,3' -> 'x,2' or 'x,3,y' -> 'x,2,y' and ditto for 4->1 i.e. ref length doesn't change and only 1 char each time
          list($broDatType, $diMeIdsA) = UnpackAmalBroDatKey($amalBroDatKey);
          foreach ($diMeIdsA as $i => $diMeId) # djh?? Only need to check first 2?
            if ($diMeId===3 || $diMeId===4) { # 3 or 4
              $diMeIdsA[$i] = $diMeId===3 ? 2 : 1; # 3->2, 4->1
              $newDatKey = substr($amalBroDatKey,0,2).implode(',', $diMeIdsA);
              if (isset($sumA[$newDatKey]))
                $sumA[$newDatKey][0] += $sumDatA[0];
              else
                $sumA[$newDatKey] = $sumDatA;
              unset($sumA[$amalBroDatKey]);
              if ($Debug) DebugMsg("Changed $amalBroDatKey to $newDatKey (Consol to Group re Target Usable Dims of $tarUsableDims");
              break;
            }
        }
      ...
      # which is OK if the DiMe is Group.Consol or Group (DiMeIds 2,1) which are the same as Consol or Consol.Consol (DiMeIds 3,4)
        ...
        if (is_string($amalBroDatKey)) { # only do DiMes i.e. skip StartBase and Base sums
          ...
          foreach ($diMeIdsA as $i => $diMeId) # djh?? Only need to check first 2?
            if ($diMeId===1 || $diMeId===2) { # 1 or 2
              $diMeIdsA[$i] = $diMeId===1 ? 4 : 3; # 1->4, 2->3
              $newDatKey = substr($amalBroDatKey,0,2).implode(',', $diMeIdsA);
              if (isset($sumA[$newDatKey]))
                $sumA[$newDatKey][0] += $sumDatA[0];
              else
                $sumA[$newDatKey] = $sumDatA;
              unset($sumA[$amalBroDatKey]);
              if ($Debug) DebugMsg("Changed $amalBroDatKey to $newDatKey (Group to Consol re Target Usable Dims of $tarUsableDims");
              break;
            }
        }
    }
    # Now check the Dim/DiMe use and adjust if necessary
    foreach ($sumA as $amalBroDatKey => $sumDatA) { # [0 => Bal, 1 => SrceN]
      if (is_string($amalBroDatKey)) { # only do DiMes i.e. skip StartBase and Base sums
        list($broDatType, $diMeIdsA) = UnpackAmalBroDatKey($amalBroDatKey);
          ...
          if ($diMeId<DiMeId_Unallocated) { # skip the unallocated cases
            ...
            if (!InChrList($dimId, $tarUsableDims) && !($broDiMesA && $broDiMesA[II_AllowsA] && in_array($diMeId, $broDiMesA[II_AllowsA]))) {
              if ($sumDatA[0] && IsPrimary($sumDatA[1])) { # skip if zero or not a primary entry but still zap the $sumA entry
                ...
                if (isset($sumA[$newDatKey])) {
                  $sumA[$newDatKey][0] += $sumDatA[0];
                  if ($Debug) DebugMsg("$sumTypeS Summing year $year $tarBroId Unallocated Bal {$sumA[$newDatKey][0]} after adding $tarBroId,$amalBroDatKey bal {$sumDatA[0]} as $amalBroDatKey (Dim $dimId) is not available in $tarBroId)");
                }else{
                  $sumA[$newDatKey] = [$sumDatA[0], BroDatSrce_S]; # [0 => Bal, 1 => SrceN]
                  ++$numDiMesA[$broDatType];
                  if ($Debug) DebugMsg("$sumTypeS Summing year $year $tarBroId Unallocated Bal {$sumDatA[0]} created from $tarBroId,$amalBroDatKey as $amalBroDatKey (Dim $dimId) is not available in $tarBroId");
                }
              }
              unset($sumA[$amalBroDatKey]);
              --$numDiMesA[$broDatType];
    ...
    foreach ([BroDatT_Start, BroDatT_End] as $broDatType) {
      if ($numDiMesA[$broDatType]===1 && isset($sumA[$unallocDatKey = "$broDatType,".DiMeId_Unallocated])) {
        ...
        if ($Debug) DebugMsg("$sumTypeS Summing year $year $tarBroId Base Bal {$sumA[$broDatType][0]} created from all DiMes -> unallocated");
        unset($sumA[$unallocDatKey]);
        --$numDiMesA[$broDatType];
      }
    }
  }
  # If there are DiMes and Base as can happen from some Bros in the sum being Base only and some with DiMes, move the Base to the default DiMe
  # Really only need to do this here if the default DiMe also has a balance as o'wise Bro::add data could handle the reallocation of Base to default.
  # But as a DefaultDiMeId() call to get the default DiMe is needed anyway, do the switch here to avoid another DefaultDiMeId() call in Bro:add data
  # (If this switch is not done here where there are both Base and default DiMe sums, on a repeat loop Bro::add data replaces the default bal twice -> 2 changes even if final result is as it was before.)
  foreach ([BroDatT_Start, BroDatT_End] as $broDatType) {
    if ($numDiMesA[$broDatType] && isset($sumA[$broDatType])) {
      $defaultDiMeId = $tarBrO->DefaultDiMeId();
      $defaultDatKey = "$broDatType,$defaultDiMeId";
      if (isset($sumA[$defaultDatKey])) {
        $sumA[$defaultDatKey][0] += $sumA[$broDatType][0];
        if ($Debug) DebugMsg("$sumTypeS Summing year $year $tarBroId Base Bal {$sumA[$broDatType][0]} -> Default DiMe $defaultDiMeId -> bal {$sumA[$defaultDatKey][0]}");
      }else{
        $sumA[$defaultDatKey] = [$sumA[$broDatType][0], BroDatSrce_S]; # [0 => Bal, 1 => SrceN]
       #++$numDiMesA[$broDatType];
        if ($Debug) DebugMsg("$sumTypeS Summing year $year $tarBroId default $defaultDiMeId Bal {$sumA[$broDatType][0]} created from Base");
      }
      unset($sumA[$broDatType]);
    }
  }
  #if ($Debug && count($sumA))  foreach ($sumA as $amalBroDatKey => $sumDatA)  DebugMsg("$tarBroId sumA[$amalBroDatKey] Bal = {$sumDatA[0]}");
  ...
  foreach ($sumA as $amalBroDatKey => $sumDatA) # [0 => Bal, 1 => SrceN]
    if (IsEndAmalBroDatKey($amalBroDatKey) || $tarIsSe) # add data if not Start or Start too if target is StartEnd
      $tarBrO($amalBroDatKey, $sumDatA[0], $sumDatA[1], 0, 0, BroAddDataOp_Replace); # add data ($broDatTypeOrBroDatKey, $dat, $srceN, $inst=0, $diMeIdOrDiMeIdsA=0, $op=BroAddDataOp_Unique)
}
...
# AddData($broId, $broDatKey, $dat, $srceN, $diMeIdOrDiMeIdsA=0)
# ==============================================================
...
# Returns the BroDat
function AddData($broId, $broDatKey, $dat, $srceN, $diMeIdOrDiMeIdsA=0) {
  global $yrBrOsA, $BrosThisDataTypeA;
  if (!isset($yrBrOsA[$broId])) {
    $yrBrOsA[$broId] = new Bro($broId);
    $BrosThisDataTypeA[$broId] = 1;
  }
  return $yrBrOsA[$broId]($broDatKey, $dat, $srceN, 0, $diMeIdOrDiMeIdsA); # add data ($broDatTypeOrBroDatKey, $dat, $srceN, $inst=0, $diMeIdOrDiMeIdsA=0, $op=BroAddDataOp_Unique)
}
...
function Movement($broId) {
  ...
  foreach ($yrBrOsA[$broId]->StartPrimaryBroDatAs() as $amalBroDatKey => $datA) { # [AmalBroDatKey => [0 => Bal, 1 => SrceN]] for the Start Summing BroDats i.e. excluding the End, Base one(s) if there are DiMes or Tuple instances, non-Primary ones, and in a Pya year, all non-Pya BroDats.
    AdjustAmalBroDatKey($amalBroDatKey, BroDatT_Start); # Start -> End
    if (isset($mvtA[$amalBroDatKey]))
      $mvtA[$amalBroDatKey] -= $datA[0];
    else
      $mvtA[$amalBroDatKey] = -$datA[0];
  }
  return $mvtA;
...
# BroEndBals($broId)
# ==================
# Returns the End balances of Bro broId, wo Base if there are DiMes, as [BroDatKey => Bal]
# or [] if the Bro is not defined
function BroEndBals($broId) {
  global $yrBrOsA;
  $balsA = [];
  if (isset($yrBrOsA[$broId]))
    foreach ($yrBrOsA[$broId]->EndPrimaryBroDatAs() as $amalBroDatKey => $bal) # [AmalBroDatKey => Bal] for the End Summing BroDats i.e. excluding the Start ones, Base one(s) if there are DiMes or Tuple instances, non-Primary ones, and in a Pya year, the non-Pya BroDats.
      $balsA[$amalBroDatKey] = $bal;
  return $balsA;
}
...
function SumList($listA) {
  ...
  foreach ($listA as $i => $broId)
    if (isset($yrBrOsA[$broId]))
      foreach ($yrBrOsA[$broId]->SummingBroDatAs() as $broDatKey => $datA) # [AmalBroDatKey => [0 => Bal, 1 => SrceN]] for the Summing BroDats i.e. excluding the Base one(s) if there are DiMes or Tuple instances, non-Primary ones, and in a Pya year, all non-Pya BroDats. Should only be called for a Non-StartEnd Summing Bro. For Tuples, the Inst # is stripped from the key and same key results wo Inst are summed.
...
function SetBroToAminusSumList($tarBroId, $aA, $listA) {
  ...
  # If there are DiMes and Base as can happen from some Bros in the sum being Base only and some with DiMes, move the Base to Unallocated.
  # djh?? If this happens often, a better method would be to go back a level and move each Base to its Default DiMe.
...
function SetDimGroups($diMeIdsA) {
  global $DiMesA, $DimGroupsA, $DGsInUseA;
  static $dimToDimGroupSA;
  # echo "SetDimGroups(",implode(',', $diMeIdsA),')<br>';
  if (!$dimToDimGroupSA) {
  ...
  foreach ($diMeIdsA as $diMeId) {
    # DiMeId -> DimId -> DimGroup
    if ($diMeId < DiMeId_Unallocated && ($dg = $dimToDimGroupSA[$DiMesA[$diMeId][DiMeI_DimId]]) >= 0)
      $DGsInUseA[$dg] = 1;
  }
}
